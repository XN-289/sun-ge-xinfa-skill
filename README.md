...
给分析型请求时，输出会写成一条连贯的案例，不堆一长串点：事实和证据标签先落地，
机制用完整句子推进，数字与出处内嵌在行文里，最后只留一段防御出口和免责声明。
只有用户明确要梗卡、清单或表格时，才用对应结构。
...
| 回收可以是非现金 | 真正的账本有七个回收项：现金、控制、流量、杠杆、地位、信息、期权 |
...
**素材不是空中楼阁。** 已收集的公开事件被蒸馏进节点：每次推导都要求先落一个可回溯事件，再标
...
[public-record.md](references/public-record.md)、[media-archive.md](references/media-archive.md) 和
[source-manifest.md](references/source-manifest.md) 中的纠错口径；仓库只做引用层，不搬运整库推文、
调研全文或私人段落。
...
| 你问 | Skill 用哪个节点 | 你会得到 |
|---|---|---|
| 这笔钱花出去值不值 | 计策沙盘 | 六问审计 + 四档结论 |
| 这篇病毒长文谁在付账 | 小作文反拆 | 八层因果 + 证据层 + 防御出口 |
| 我的内容 / 项目怎么扩散 | 炒作编排 | 七阶段链路 + 注意力三桶账 |
| 我亏钱后总想一把回本 | 防割清单 | 十条冷却链 + 三行回复 |
| “孙哥会怎么拿捏人” | 黑箱拆解 | 机制、伤害、目标防御，不写步骤 |
| 现在这个热点有哪些可查事实 | 外部搜索 + 案例研究 | 时间线 / 矩阵 + 来源强度 |

所以一键安装拿到的是三样：**看穿结构**、**守住边界**、**把一次热闹沉淀成可复用资产**。
